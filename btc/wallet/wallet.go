...
func (wal *wallet) Execute(ctx context.Context, actions []btc.HtlcAction, tx *wire.MsgTx) (*wire.MsgTx, error) {
	wal.mu.Lock()
	defer wal.mu.Unlock()

	initiates, redeems, refunds, instantRefunds := btc.ParseHtlcActions(actions)

	return wal.execute(ctx, actions, tx)
}
