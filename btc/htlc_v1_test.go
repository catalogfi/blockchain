...
// 			script, err := btc.MultisigScript(key1.PubKey().SerializeCompressed(), key2.PubKey().SerializeCompressed())
...
//
// 			By("Construct the funding tx (addr1 -> multi-sig)")
...
// 			Expect(signers.Sign(fundingTx)).Should(Succeed())
...
// 			for i := range utxos1 {
// 				sig1, err := txscript.RawTxInWitnessSignature(redeemTx, txscript.NewTxSigHashes(redeemTx, fetcher), i, amount, script, txscript.SigHashAll, key1)
// 				Expect(err).To(BeNil())
// 				sig2, err := txscript.RawTxInWitnessSignature(redeemTx, txscript.NewTxSigHashes(redeemTx, fetcher), i, amount, script, txscript.SigHashAll, key2)
// 				Expect(err).To(BeNil())
